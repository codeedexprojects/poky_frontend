        ...
        <Menu>
            ...
            <MenuList>
                <Link to='/user-profile' className='outline-none'>
                    <MenuItem className="flex items-center gap-2 text-xl hover:!text-black">
                        <HiMiniUserCircle />
                        <Typography variant="small" className="font-medium font-custom flex items-center gap-2">
                            My Profile
                            {userCoupon && (<div className='w-3 h-3 bg-black rounded-full'></div>)}
                        </Typography>
                    </MenuItem>
                </Link>
                <hr className="my-2 border-blue-gray-50" />
                <Link to='/login-user' className='outline-none'>
                    <MenuItem onClick={handleLogout} className="flex items-center gap-2 text-xl hover:!text-black">
                        <FaSignOutAlt />
